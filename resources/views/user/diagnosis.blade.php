...
@section('content')
            ...
            <div class="max-w-3xl mx-auto bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
                ...
                <form id="diagForm" enctype="multipart/form-data">
                    ...
                    <div class="step-panel p-8 hidden" id="step-2">
                        ...
                        <div class="flex justify-between mt-8">
                            ...
                            <button type="button" class="px-8 py-3 bg-emerald-600 text-white rounded-full font-bold shadow-lg transition-all transform hover:-translate-y-1" onclick="goStep(3)">
                                Selanjutnya <i class="fa-solid fa-arrow-right ml-2"></i>
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="step-panel p-8 hidden" id="step-3">
                        ...
                        <div class="dropzone border-2 border-dashed border-slate-200 rounded-2xl p-10 text-center bg-slate-50 hover:bg-emerald-50 hover:border-emerald-300 transition-all cursor-pointer group" onclick="document.getElementById('file-input').click()">
                            <i class="fa-solid fa-cloud-arrow-up text-4xl text-slate-300 mb-4 block group-hover:text-emerald-500"></i>
                        ...
                        <div class="flex justify-between mt-8">
                            ...
                            <button type="button" class="px-8 py-3 bg-emerald-600 text-white rounded-full font-bold shadow-lg transition-all transform hover:-translate-y-1" onclick="goStep(4)">
                                Selanjutnya <i class="fa-solid fa-arrow-right ml-2"></i>
                            </button>
                        </div>
                    </div>
                ...
                <div id="result" class="hidden p-8 animate-fade-in">
                    ...
                    <div id="repair-area" class="hidden">
                        <div id="repair-estimate-box" class="mb-6 overflow-hidden rounded-2xl border-2 border-emerald-100 bg-white">
                            <div class="bg-emerald-600 px-4 py-2 text-white text-[10px] font-black uppercase tracking-[0.2em] text-center">Repair Estimation Detail</div>
                        ...
                        <div class="bg-slate-50 p-6 rounded-2xl border border-slate-200 text-center mb-6">
                            <p class="text-slate-600 text-sm mb-4">Kabar baik! Perangkat Anda masih sangat layak untuk diperbaiki.</p>
                            <button id="action-btn-repair" class="w-full px-10 py-4 rounded-full text-white font-bold shadow-xl bg-gradient-to-r from-emerald-600 via-emerald-500 to-lime-500 hover:from-emerald-700 hover:to-lime-600 transform hover:scale-[1.02] transition-all duration-300 flex items-center justify-center gap-2">
                                <i class="fa-solid fa-wand-magic-sparkles"></i> Lihat Teknisi Sekarang
                            </button>
                        </div>
                    </div>

                    {{-- Donation Area (FIXED WITH DYNAMIC HUBS) --}}
                    <div id="donation-area" class="hidden">
                        <div class="bg-amber-50 border border-amber-100 p-4 rounded-2xl flex gap-3 mb-6">
                            ...
                            <p class="text-xs text-amber-800 leading-relaxed">
                                <strong>Barang ini masih bisa menjadi sumber spare part berharga.</strong> Melalui donasi ke Fixora Hub, komponen yang masih berfungsi akan didistribusikan ke komunitas repair.
                            </p>
                        </div>

                        <div class="flex gap-1 bg-slate-100 p-1 rounded-xl mb-6 pointer-events-none">
                            <button type="button" id="tab-btn-hub" class="don-tab-btn active flex-1 py-2 text-[10px] font-bold uppercase rounded-lg transition-all">1. Pilih Hub</button>
                            <button type="button" id="tab-btn-method" class="don-tab-btn flex-1 py-2 text-[10px] font-bold uppercase rounded-lg transition-all">2. Metode</button>
                            <button type="button" id="tab-btn-scan" class="don-tab-btn flex-1 py-2 text-[10px] font-bold uppercase rounded-lg transition-all">3. Konfirmasi</button>
                        </div>

                        <div id="don-panel-hub" class="don-panel space-y-3">
                            <div class="relative mb-4">
                                <i class="fa-solid fa-magnifying-glass absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
                                <input type="text" id="hub-search" onkeyup="filterHubs()" placeholder="Cari lokasi hub terdekat..." class="w-full pl-10 pr-4 py-3 rounded-xl border-2 border-slate-100 focus:border-emerald-500 text-sm">
                            </div>

                            <div id="hub-list-container" class="space-y-3 max-h-[250px] overflow-y-auto pr-1">
                                @forelse($hubs as $hub)
                                    <div class="hub-card p-4 border-2 {{ $loop->first ? 'border-emerald-500 bg-emerald-50 active-hub' : 'border-slate-100' }} rounded-2xl cursor-pointer transition-all"
                                        onclick="selectHub(this, '{{ $hub->nama_toko }}')">
                                        <div class="flex justify-between items-start">
                                            <div class="flex gap-3">
                                                <div class="icon-box w-10 h-10 {{ $loop->first ? 'bg-white text-emerald-600' : 'bg-slate-50 text-slate-400' }} rounded-xl flex items-center justify-center shadow-sm transition-colors">
                                                    <i class="fa-solid fa-warehouse"></i>
                                                </div>
                                                <div>
                                                    <h5 class="hub-name text-sm font-bold text-slate-800">{{ $hub->nama_toko }}</h5>
                                                    <p class="text-[10px] text-slate-500 mb-2">{{ $hub->alamat }}, {{ $hub->kota }}</p>
                                                    <div class="flex gap-2">
                                                        <span class="px-2 py-0.5 bg-emerald-100 text-emerald-700 text-[9px] font-bold rounded-md">AKTIF</span>
                                                        <span class="text-[9px] text-slate-400 font-bold"><i class="fa-solid fa-star text-amber-400 mr-1"></i>{{ $hub->rating }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <i class="check-icon fa-solid fa-circle-check {{ $loop->first ? 'text-emerald-500' : 'text-slate-200' }} transition-colors"></i>
                                        </div>
                                    </div>
                                @empty
                                    <div class="text-center py-10 bg-slate-50 rounded-2xl">
                                        <p class="text-xs text-slate-400 font-bold uppercase tracking-widest">Hub Tidak Tersedia</p>
                                    </div>
                                @endforelse
                            </div>
                            <button type="button" onclick="switchDonTab('method')" class="w-full mt-4 py-4 bg-emerald-600 text-white rounded-2xl text-xs font-bold shadow-lg transform active:scale-95 transition-transform">Lanjut Pilih Metode</button>
                        </div>

                        <div id="don-panel-method" class="don-panel hidden space-y-4 animate-fade-in">
                            <div class="grid grid-cols-2 gap-3">
                                <div class="method-card p-6 border-2 border-emerald-500 bg-emerald-50 rounded-2xl text-center cursor-pointer transition-all" onclick="selectMethod(this, 'Pickup')">
                                    <div class="w-12 h-12 bg-white rounded-full flex items-center justify-center text-emerald-600 shadow-sm mx-auto mb-3">
                                        <i class="fa-solid fa-truck"></i>
                                    </div>
                                    <span class="block text-xs font-bold text-slate-800">Pickup Rumah</span>
                                </div>
                                <div class="method-card p-6 border-2 border-slate-100 rounded-2xl text-center cursor-pointer transition-all" onclick="selectMethod(this, 'Drop-off')">
                                    <div class="w-12 h-12 bg-white rounded-full flex items-center justify-center text-slate-400 shadow-sm mx-auto mb-3">
                            ...
                            <div class="bg-slate-900 p-5 rounded-2xl text-white flex items-center gap-4">
                                <div class="w-12 h-12 bg-emerald-500/20 rounded-xl flex items-center justify-center text-emerald-400 text-xl">
                                    <i class="fa-solid fa-gift"></i>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold uppercase text-emerald-400 tracking-widest">Circular Bonus</p>
                                    <p class="text-sm font-bold">Anda akan menerima +150 Fixora Points</p>
                                </div>
                            </div>
                            <button type="button" onclick="switchDonTab('scan')" class="w-full py-4 bg-emerald-600 text-white rounded-2xl text-xs font-bold shadow-lg">Generate Barcode</button>
                        </div>
                        ...
                        <div id="don-panel-scan" class="don-panel hidden text-center animate-fade-in">
                            <div class="bg-slate-900 p-8 rounded-[2.5rem] mb-6 relative overflow-hidden">
                                ...
                                <p class="text-[10px] text-emerald-400 font-bold mb-6 tracking-[0.3em] uppercase">E-Waste Clearance ID</p>
                                <div class="bg-white p-5 rounded-3xl inline-block mb-6 shadow-2xl">
                                    <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=FIXORA-DONATE" alt="QR" class="w-36 h-36">
                                </div>
                                <p id="bc-display" class="font-mono text-white text-sm tracking-[0.4em] font-bold">FXR-2026-XXXX</p>
                            </div>
                            <button type="button" onclick="simulateScan()" id="sim-btn" class="w-full py-4 border-2 border-emerald-500 text-emerald-600 rounded-2xl text-xs font-bold hover:bg-emerald-50 transition-all">Selesaikan Donasi Sekarang</button>

                            <div id="pts-card" class="hidden mt-6 animate-fade-in bg-white border-2 border-emerald-100 p-6 rounded-3xl text-left shadow-xl">
                                <div class="flex items-center gap-4 mb-4">
                                    <div class="w-12 h-12 rounded-2xl bg-emerald-500 flex items-center justify-center text-white text-xl shadow-lg">
                                        <i class="fa-solid fa-star"></i>
                                    </div>
                                    <div>
                                        <h5 class="text-sm font-black text-slate-800 uppercase tracking-tight">Mission Accomplished!</h5>
                                        <p class="text-[10px] text-slate-500 font-bold uppercase">Reward: Circular Champion</p>
                                    </div>
                                </div>
                                <div class="text-4xl font-black text-emerald-600 mb-3">+150 <span class="text-sm font-bold text-slate-400">Pts</span></div>
                    ...
                    <div class="text-center mt-8">
                        <button type="button" onclick="location.reload()" class="text-slate-400 text-[12px] font-bold hover:text-emerald-600 hover:underline transition-all">
    ...
    <script>
        ...
        function goStep(step) {
            ...
            for (let i = 1; i <= 4; i++) {
                ...
                if (i < currentStep) {
                    ...
                    numEl.className = "step-num w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold bg-emerald-500 text-white";
                    ...
                    numEl.className = "step-num w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold bg-slate-200 text-slate-500";
                    numEl.innerHTML = i;
                }
            }
        }
        ...
        function switchDonTab(tab) {
            ...
            document.querySelectorAll('.don-tab-btn').forEach(b => b.classList.remove('active', 'bg-white', 'text-emerald-600', 'shadow-sm'));
        ...
        function selectHub(el, name) {
            document.querySelectorAll('.hub-card').forEach(c => {
                c.classList.remove('border-emerald-500', 'bg-emerald-50', 'active-hub');
                c.classList.add('border-slate-100');
                
                const iconBox = c.querySelector('.icon-box');
                iconBox.classList.remove('bg-white', 'text-emerald-600');
                iconBox.classList.add('bg-slate-50', 'text-slate-400');

                c.querySelector('.check-icon').classList.replace('text-emerald-500', 'text-slate-200');
            });

            el.classList.remove('border-slate-100');
            el.classList.add('border-emerald-500', 'bg-emerald-50', 'active-hub');
            ...
            el.querySelector('.check-icon').classList.replace('text-slate-200', 'text-emerald-500');
        }
        ...
        function selectMethod(el, name) {
            document.querySelectorAll('.method-card').forEach(card => {
                ...
                card.querySelector('.fa-solid').parentElement.classList.replace('text-emerald-600', 'text-slate-400');
            });
            el.classList.remove('border-slate-100');
        ...
        document.getElementById('diagForm').addEventListener('submit', function(e) {
            ...
            .then(data => {
                setTimeout(() => {
                    ...
                    document.getElementById('cost-val').innerText = data.cost_saved || '65%';
                    document.getElementById('kg-val').innerText = data.kg_saved || '2.4kg';
                    document.getElementById('co2-val').innerText = data.emission_saved || '1.8kg';
                    ...
                    if (data.is_repairable) {
                        ...
                        document.getElementById('res-price').innerText = data.repair_price || 'Rp 150.000';
                        document.getElementById('res-duration').innerText = data.repair_duration || '2 Hari';
                        document.getElementById('res-status-badge').innerText = "Layak Repair";
                        document.getElementById('res-status-badge').className = "px-4 py-2 rounded-full text-[10px] font-black uppercase bg-emerald-50 text-emerald-600 border border-emerald-200";
                        document.getElementById('action-btn-repair').onclick = () => {
                            window.location.href = `{{ route('booking.index') }}?diag_id=${data.diag_id}&device_name=${encodeURIComponent(data.device_name)}`;
                        };
                    } else {
                        ...
                        document.getElementById('res-status-badge').className = "px-4 py-2 rounded-full text-[10px] font-black uppercase bg-amber-50 text-amber-600 border border-amber-200";
                    }
                }, 1500);
            })
            .catch(err => {
    ...
    <style>
        ...
        .don-tab-btn.active { background-color: white; color: #059669; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); }
        #hub-list-container::-webkit-scrollbar { width: 4px; }
        #hub-list-container::-webkit-scrollbar-thumb { background: #e2e8f0; border-radius: 10px; }
    </style>
@endsection
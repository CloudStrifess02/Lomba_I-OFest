...
{{-- Penambahan px-8 dan lg:px-20 di sini agar kiri-kanan tidak mepet layar --}}
<div class="flex-1 flex flex-col justify-center items-center min-h-screen px-8 lg:px-20 py-12">
    
    {{-- Lebar Maksimal diturunkan menjadi max-w-[960px] agar lebih rapi --}}
    <div class="w-full max-w-[960px] flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-16 z-10">

        {{-- ═════ BAGIAN KIRI (TEKS) ═════ --}}
        <div class="flex flex-col w-full lg:flex-1 max-w-md">
            ...
            {{-- Teks H1 diperkecil dari 5xl ke 4xl untuk desktop --}}
        ...
        {{-- Lebar kartu desktop disesuaikan menjadi 380px agar proporsional --}}
        <div class="w-full max-w-md lg:w-[380px] shrink-0 bg-white rounded-[1.5rem] p-6 lg:p-8 border border-emerald-50" style="box-shadow: 0 20px 40px rgba(0,0,0,.04), 0 1px 3px rgba(0,0,0,.02);">
            <div class="w-full">
                <h1 class="text-xl lg:text-2xl font-extrabold text-center text-gray-800 mb-2">Masuk ke <span class="text-emerald-600">Fixora</span></h1>
                <p class="text-center text-xs lg:text-sm text-gray-500 mb-6 lg:mb-8">Selamat datang kembali! Silakan masuk untuk melanjutkan.</p>

                @if (session('status'))
                    <div class="mb-4 p-3 bg-emerald-100 text-emerald-700 rounded-xl text-xs sm:text-sm">{{ session('status') }}</div>
                @endif
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-4">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <div class="relative">
                            ...
                            <input type="email" id="email" name="email" class="w-full pl-9 pr-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition text-sm @error('email') border-red-500 @else border-gray-200 @enderror" placeholder="Masukkan email Anda" value="{{ old('email') }}" required autofocus>
                        </div>
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <div class="relative">
                            ...
                            <input type="password" id="password" name="password" class="w-full pl-9 pr-10 py-2.5 border rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition text-sm @error('password') border-red-500 @else border-gray-200 @enderror" placeholder="Masukkan password" required>
                        ...
                        <div class="text-right mt-1">
                            @if (Route::has('password.request'))
                                <a href="{{ route('password.request') }}" class="text-xs text-emerald-600 hover:underline font-semibold">Lupa password?</a>
                            @endif
                        </div>
                    </div>

                    <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 rounded-xl shadow-lg shadow-emerald-200 transition-all active:scale-95 mt-2 text-sm">Masuk Sekarang</button>
                </form>
    ...
    {{-- Trust Bar digeser sedikit ke kanan (left-12) agar seimbang --}}

@foreach($toko as $t)
    @php
        $kategori = strtolower($t->kategori->nama_kategori ?? '');
        ...
        if ($t->foto_cover) {
            $coverUrl = str_starts_with($t->foto_cover, 'http') 
                ? $t->foto_cover 
                : asset('storage/' . $t->foto_cover);
        }
    @endphp
    ...
    <div class="store-card group bg-white rounded-[28px] overflow-hidden border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.15)] transition-all duration-500 hover:-translate-y-2 flex flex-col h-full">
        ...
        <a href="{{ route('marketplace.detail', $t->id) }}" class="h-36 w-full relative overflow-hidden bg-slate-200 shrink-0 block">
            <img src="{{ $coverUrl }}"
                alt="Cover {{ $t->nama_toko }}"
            ...
            <span class="absolute top-4 right-4 bg-white/95 backdrop-blur shadow-sm {{ $theme['text'] }} text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1">
                <i class="fa-solid {{ $theme['icon'] }}"></i> {{ $t->kategori->nama_kategori ?? 'Mitra' }}
            </span>
        </a>
        ...
        <div class="px-6 pb-6 flex flex-col flex-grow">
            ...
            <div class="w-16 h-16 rounded-2xl bg-white p-1.5 shadow-md relative -mt-8 mb-4 border border-slate-50 z-10 shrink-0">
                @if($t->logo_toko)
                    <img src="{{ str_starts_with($t->logo_toko, 'http') ? $t->logo_toko : asset('storage/' . $t->logo_toko) }}" alt="Logo" class="w-full h-full rounded-xl object-cover">
                @else
                    <div class="w-full h-full rounded-xl bg-gradient-to-br {{ $theme['gradient'] }} flex items-center justify-center text-white font-bold text-xl">
                        {{ strtoupper(substr($t->nama_toko, 0, 2)) }}
                    </div>
                @endif
            </div>
            ...
            <div class="mb-5 flex justify-between items-start gap-2">
                <div>
                    <a href="{{ route('marketplace.detail', $t->id) }}">
                        <h3 class="font-extrabold text-xl text-slate-800 {{ $theme['text'] }} transition-colors leading-tight hover:underline">
                            {{ $t->nama_toko }}
                        </h3>
                    </a>
                    <p class="text-slate-500 text-sm mt-1.5 flex items-center gap-1.5">
                        <i class="fa-solid fa-location-dot text-slate-400"></i> {{ ucfirst($t->alamat) }}
                    </p>
                </div>
                ...
                @else
                    <div class="flex items-center gap-1 bg-amber-50 px-2 py-1 rounded-lg text-amber-600 font-bold text-sm shrink-0">
                        <i class="fa-solid fa-star text-xs"></i> {{ $t->rating }}
                    </div>
                @endif
            </div>

            <div class="space-y-3 mb-6">
                @foreach($t->produk->take(2) as $item)
            ...
            <a href="{{ route('marketplace.detail', $t->id) }}" class="mt-auto w-full {{ $theme['bg_light'] }} {{ $theme['bg_hover'] }} {{ $theme['text'] }} hover:text-white font-bold py-3.5 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 text-center">

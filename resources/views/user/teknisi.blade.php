...
@section('content')
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Syne:wght@700;800&family=Manrope:wght@400;500;600;700;800&display=swap"
        rel="stylesheet">
    ...
    <div class="hero-bg mt-6">
        <div class="container mx-auto px-6 text-center">
            <div
                class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white border border-emerald-100 text-emerald-600 text-[10px] font-bold uppercase tracking-widest mb-4">
                <i class="fa-solid fa-screwdriver-wrench"></i> Expert Network
            </div>
            <h1 class="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter font-poppins mb-4">
                Pilih <span class="text-emerald-600">Teknisi Terbaik</span>
            </h1>
            <p class="text-slate-500 max-w-md mx-auto">
                Berdasarkan diagnosis <strong
                    class="text-slate-800">#{{ $diagnosis['diag_id'] ?? request('diag_id', 'FXR-2026') }}</strong>,
    ...
    <main class="container mx-auto px-6 py-12">
        ...
        <div class="mb-8 bg-white p-5 rounded-3xl border border-slate-200 shadow-sm">
            <div class="flex flex-col lg:flex-row gap-4">
                <div class="flex-grow relative">
                    ...
                    <input type="text" id="search-tech" onkeyup="filterTechnicians()" placeholder="Cari nama toko..."
                ...
                <div class="flex items-center gap-3 px-4 py-3 bg-slate-50 rounded-2xl border-2 border-slate-50">
                    ...
                    <input type="checkbox" id="filter-verified" onchange="filterTechnicians()"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-4 overflow-y-auto pr-2 custom-scrollbar" style="max-height: 680px;"
                    ...
                    <div class="tech-card p-6 flex flex-col sm:flex-row items-start sm:items-center gap-6"
                        ...
                        <div class="flex-grow">
                            ...
                            <div class="flex items-center gap-4 text-sm text-slate-400">
                                <span class="flex items-center"><i class="fa-solid fa-star text-amber-400 mr-1"></i>
                                    {{ number_format($item->rating, 1) }}</span>
                                <span class="text-slate-300">|</span>
                                <span
                                    class="text-indigo-500 font-semibold">{{ $item->kategori->nama_kategori ?? 'Umum' }}</span>
                            </div>
                        </div>
                ...
                @empty
                    <div
                        class="text-center py-20 bg-white rounded-3xl border border-dashed border-slate-200 text-slate-400">
                ...
                <div class="booking-panel bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden"
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex items-center gap-4 p-4 rounded-2xl bg-emerald-50 border border-emerald-100 mb-6">
                            <div
                                class="w-10 h-10 rounded-xl bg-white border border-emerald-200 flex items-center justify-center text-lg">
                                📦</div>
                            <div>
                                <p class="text-[10px] text-emerald-600 font-bold uppercase tracking-tighter">Diagnosa Aktif
                                </p>
                                <p class="text-sm font-bold text-slate-800">{{ $diagnosis['device_name'] ?? 'Perangkat' }}
                                </p>
                            </div>
                        </div>
                        ...
                        {{-- State Kosong --}}
                        <div id="no-selection"
                            class="{{ old('toko_id') ? 'hidden' : '' }} py-12 text-center border-2 border-dashed border-slate-100 rounded-3xl">
                            <i class="fa-solid fa-store text-slate-200 text-3xl mb-3"></i>
                            <p class="text-slate-400 text-xs px-10">Pilih salah satu toko di sebelah kiri untuk melanjutkan
                            </p>
                        </div>
                        ...
                        <div id="booking-form-area" class="{{ old('toko_id') ? '' : 'hidden' }} animate-fade-in">
                            <form action="{{ route('booking.store') }}" method="POST">
                                ...
                                <input type="hidden" name="diag_id"
                                    value="{{ $diagnosis['diag_id'] ?? request('diag_id') }}">

                                <div class="space-y-5">
                                    <div class="p-4 rounded-2xl bg-slate-50 border border-slate-100">
                                        <p class="text-[10px] text-slate-400 font-bold uppercase mb-1">Toko Terpilih</p>
                                        <p id="display-tech-name" class="font-black text-slate-800 text-base">
                                            {{ old('tech_name_display', 'Nama Toko') }}</p>
                                        <input type="hidden" name="tech_name_display" id="input-tech-name-display"
                                            value="{{ old('tech_name_display') }}">
                                    </div>

                                    <div>
                                        <label
                                            class="block text-[10px] font-bold text-slate-400 uppercase mb-2 ml-1">Jadwal</label>
                                        <input type="datetime-local" name="schedule" value="{{ old('schedule') }}"
                                            class="w-full p-4 rounded-xl border-2 border-slate-100 focus:border-emerald-500 outline-none transition-all text-sm"
                                            required>
                                    </div>

                                    <div>
                                        <label class="block text-[10px] font-bold text-slate-400 uppercase mb-2 ml-1">Alamat
                                            Penjemputan</label>
                                    ...
                                    <button type="submit"
                                        class="w-full py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black shadow-lg transition-all transform hover:-translate-y-1">
                                        Konfirmasi Booking
                                    </button>
                                </div>
                            </form>
